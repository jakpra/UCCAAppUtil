...
#EXT_TO_INT_IDS["lpp_1943_1.1-10"] = 3094
#EXT_TO_INT_IDS["lpp_1943_1.11-20"] = 3095
#EXT_TO_INT_IDS["lpp_1943_1.21-30"] = 3096
#EXT_TO_INT_IDS["lpp_1943_1.31-35"] = 3097
#EXT_TO_INT_IDS["lpp_1943_4.1-10"] = 3098
#EXT_TO_INT_IDS["lpp_1943_4.11-20"] = 3099
#EXT_TO_INT_IDS["lpp_1943_4.21-30"] = 3100
#EXT_TO_INT_IDS["lpp_1943_4.31-40"] = 3101
#EXT_TO_INT_IDS["lpp_1943_4.41-50"] = 3102
#EXT_TO_INT_IDS["lpp_1943_4.51-62"] = 3103
EXT_TO_INT_IDS["lpp_1943_1.1"] = 3094
EXT_TO_INT_IDS["lpp_1943_1.2"] = 3095
EXT_TO_INT_IDS["lpp_1943_1.3"] = 3096
EXT_TO_INT_IDS["lpp_1943_1.4"] = 3097
EXT_TO_INT_IDS["lpp_1943_4.1"] = 3098
EXT_TO_INT_IDS["lpp_1943_4.2"] = 3099
EXT_TO_INT_IDS["lpp_1943_4.3"] = 3100
EXT_TO_INT_IDS["lpp_1943_4.4"] = 3101
EXT_TO_INT_IDS["lpp_1943_4.5"] = 3102
EXT_TO_INT_IDS["lpp_1943_4.6"] = 3103
EXT_TO_INT_IDS["lpp_1943_2.1"] = 3104
EXT_TO_INT_IDS["lpp_1943_2.2"] = 3105
EXT_TO_INT_IDS["lpp_1943_2.3"] = 3106
EXT_TO_INT_IDS["lpp_1943_2.4"] = 3107
EXT_TO_INT_IDS["lpp_1943_2.5"] = 3108
EXT_TO_INT_IDS["lpp_1943_2.6"] = 3109
EXT_TO_INT_IDS["lpp_1943_2.7"] = 3110
EXT_TO_INT_IDS["lpp_1943_bl.1"] = 3111
EXT_TO_INT_IDS["lpp_1943_bl.2"] = 3112
EXT_TO_INT_IDS["lpp_1943_bl.3"] = 3113
EXT_TO_INT_IDS["lpp_1943_bl.4"] = 3114
EXT_TO_INT_IDS["lpp_1943_5.1"] = 3116
EXT_TO_INT_IDS["lpp_1943_5.2"] = 3117
EXT_TO_INT_IDS["lpp_1943_5.3"] = 3118
EXT_TO_INT_IDS["lpp_1943_5.4"] = 3119
EXT_TO_INT_IDS["lpp_1943_5.5"] = 3120
EXT_TO_INT_IDS["lpp_1943_5.6"] = 3121
...
#mwe_layer = {"id":64,"name":"Multiword Expressions","type":"ROOT"}
...
#ref_layer = {"id":76,"name":"SNACS all backtick Means Locus","type":"REFINEMENT"}
...
class SSTPassage:
    def __init__(self, passage_id, filename, format="sst", upload=False, project=78, source=13, tok_task=None, ann_task=None, ref_task=None, tokenize=True, annotate=True, refine=True, username="", password="", int_id="", verbose=False):
        self.sentences = []
        self.ID = passage_id
        if int_id:
            self.ID = int_id
        else:
            self.ID = EXT_TO_INT_IDS[passage_id]
...
#         self.ref_task = ref_task
        ...
        with open(filename) as f:
            ...
            for line in f:
                line = line.strip()
                if line:
                    sent = self.read_line(line, format=format) #filename.rsplit(".")[-1])
                    sent["offset"] = local_offset
                    self.sentences.append(sent)
                    self.offsets.append(local_offset)
                    local_offset += len(sent["words"]) + 1 # +1 for linebreak token
        ...
        if upload:
            ...
            self.passage = self.server_accessor.get_passage(self.ID)
            self.user = self.server_accessor.login(username, password)['profile']
        else:
            ...
            if upload:
                if tok_task == None:
                    ...
                    'passage': self.passage, \
                     'is_demo': False, \
                     'manager_comment': 'automatically uploaded', \
                     'user_comment': '', \
                     'out_of_date': False, \
                     'obseleted_by': None, \
...
#                    self.tokens = self.server_accessor.get_user_task(self.tok_task["id"])["tokens"]
                else:
    ...
    def read_line(self, line, format="sst"):
        if format=="sst":
            sent_id, _, _json = line.strip().split("\t")
            d = json.loads(_json)
            d["text"] = " ".join([word[0] for word in d["words"]])
            d["id"] = sent_id
        elif format=="conllulex":
            assert False, "input format {} currently not supported".format(format)
        else:
            d = {}
            d["text"] = line.strip()
            d["words"] = d["text"].split()
            d["id"] = ""
        return d
...
#                             'task_id': self.ann_task["id"], \
        ...
        first_tok_id = self.user_tok_task["tokens"][0]["id"] if self.user_tok_task else self.tokens[0]["id"]
...
#                             'task_id': self.ann_task["id"], \
                ...
                # strong expressions
                if int_tok_id not in seen and (not self.user_tok_task or self.user_tok_task["tokens"][sent["offset"]+ext_tok_id-1]["require_annotation"] == True):
...
def main():
    ...
    parser.add_argument('-I', '--app-id', type=str, help='the passage ID as assigned in UCCAApp')
    ...
    parser.add_argument('-f', '--format', type=str, help='input format; one of {sst, conllulex, txt}')
